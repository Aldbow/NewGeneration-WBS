...
export async function deleteTicketFromDb(
  ticketUuid: string
): Promise<{ success: boolean; error?: string }> {
  try {
    // Delete from tickets table (CASCADE will handle declarations, wbs_reports, timelines)
    ...
    // Also attempt to clean up storage files (best effort)
    try {
      ...
      if (sigFiles && sigFiles.length > 0) {
        const sigPaths = sigFiles.map((f) => `${ticketUuid}/${f.name}`)
      ...
      if (evidenceFiles && evidenceFiles.length > 0) {
        const evidencePaths = evidenceFiles.map((f) => `${ticketUuid}/${f.name}`)
        await supabase.storage.from('wbs-evidence').remove(evidencePaths)
      }
    } catch {
      // Storage cleanup failure is not critical
      console.warn('Storage cleanup failed for ticket:', ticketUuid)
    }

    return { success: true }
  } catch (err) {
    console.error('Unexpected error in deleteTicket:', err)
    return { success: false, error: 'Terjadi kesalahan tidak terduga' }
  }
}

// ──────────────────────────────────────────────
// ADMIN: GET SIGNATURE URL
// ──────────────────────────────────────────────
...
export async function getEvidenceFiles(
  ...
  try {
    ...
    console.error('Error getting evidence files:', err)
    return []
  }
}
...
export function exportTicketsToCsv(
  ...
  if (type === 'deklarasi') {
    headers = ['No. Tiket', 'Nama', 'NIP', 'Jabatan', 'Unit', 'Email', 'No. HP', 'Status', 'Urgensi', 'Tanggal Dibuat']
    rows = filtered.map((t) => [
      t.ticketId,
      t.nama || '',
      t.nip || '',
    ...
    headers = ['No. Tiket', 'Kategori', 'Judul', 'Tanggal Kejadian', 'Lokasi', 'Urgensi', 'Status', 'Jumlah Bukti', 'Tanggal Dibuat']
    rows = filtered.map((t) => [
      t.ticketId,
      t.category || '',
      t.title || '',
      ...
      t.urgency,
      t.status,
      String(t.filesCount || 0),
      new Date(t.createdAt).toLocaleDateString('id-ID'),
    ])
  } else {
    headers = ['No. Tiket', 'Jenis', 'Detail', 'Status', 'Urgensi', 'Tanggal Dibuat']
    rows = filtered.map((t) => [
      t.ticketId,
      t.type === 'deklarasi' ? 'Deklarasi' : 'Laporan WBS',
      t.type === 'deklarasi' ? (t.nama || '') : (t.title || ''),
      t.status,
      t.urgency,
      new Date(t.createdAt).toLocaleDateString('id-ID'),
    ])
  ...
  link.href = url
  link.download = `${type === 'all' ? 'semua-tiket' : type === 'deklarasi' ? 'deklarasi' : 'laporan-wbs'}_${new Date().toISOString().slice(0, 10)}.csv`
